...
pattern = r'(?P<ip>[\d.:a-fA-F]+)\s+-\s+[^\s]+\s+\[(?P<timestamp>[^\]]+)\]\s+"(?P<method>\w+)\s(?P<url>\S+)\s(?P<http_version>[^"]+)"\s+(?P<status>\d+)\s+(?P<size>\d+|-)\s+"[^"]*"\s+"(?P<user_agent>[^"]*)"(?:\s+"[^"]*")?'
...
if not predict_one_url:
    raise ValueError("PREDICT_ONE_URL is not set. Add it to your .env file.")

if not os.path.exists(filename):
    raise FileNotFoundError(
        f"Access log not found at '{filename}'. Set NOGINX_ACCESS_LOG_PATH in .env to your active access log file."
    )
...
with open(filename, 'r') as file:
    ...
    while True:
        ...
        if match:
            ...
            print(f"Traffic seen: {df['status'].values[0]} {cur_method.values[0]} {cur_path.values[0]}")
